...
class CustomDatasetTest(unittest.TestCase):
    ...
    def test_length(self):
        expected_length = 10
        self.assertEqual(self.data_length,expected_length , msg="Length of datasets must be 10")

    def test_transforms(self):


        def check_types(expected_image, expected_label):
            self.assertIsInstance(expected_image, torch.Tensor,
                                  msg=f"Data must be a tensor but received {type(expected_image)}")
            self.assertIsInstance(expected_label, np.int64,
        ...
        for index in range(self.data_length):
            ...
            self.assertTrue(are_equal , msg="Normalization is not correctly done! Normalization must be done with transforms.Normalize((0.5,), (0.5,)).")
