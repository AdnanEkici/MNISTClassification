...
def train(model, train_loader, val_loader, criterion, tensorboard_writer, learning_rate=0.001, save_check_point=1,
    ...
    for epoch in tqdm(range(1, num_epochs + 1), colour="magenta"):
        ...
        if save_check_point != 0 and epoch % save_check_point == 0:
            ...
            torch.save(model.state_dict(),
                       'saved_mnist_classifier_models' + os.sep + f'{str(epoch)}_mnist_classifier.pth')
        ...
        validation(model=model, loader=val_loader, tensorboard_writer=tensorboard_writer,
                   device=device) if val_loader is not None else None

    tensorboard_writer.close()


def validation(model, loader, tensorboard_writer=None,
               device=torch.device("cuda" if torch.cuda.is_available() else "cpu")):
    correct = 0
    total = 0
    with torch.no_grad():
    ...
    tensorboard_writer.add_scalar("Accuracy/validation", validation_accuracy,
                                  0) if tensorboard_writer is not None else None
...
if __name__ == '__main__':
    ...
    validation_dataset_path = train_config[
        "validation_dataset_path"] if "validation_dataset_path" in train_config else None
    ...
    val_loader = DataLoader(dataset, batch_size=batch_size,
                            shuffle=True) if validation_dataset_path is not None else None
    ...
    train(model=model, train_loader=train_loader, val_loader=val_loader, criterion=criterion,
          learning_rate=learning_rate, save_check_point=save_check_point, tensorboard_writer=tensorboard_writer,
          device=device)
